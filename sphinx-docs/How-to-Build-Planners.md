...
As of Caldera V4.0, a new service has been added to the core of Caldera for use with planners and other components that make use of facts: the Knowledge Service. This service provides a variant of the functionality present in the object store utilized throughout Caldera, specifically tailored to facts, relationships, and rules.
...
The Knowledge Service stores data persistently in the same manner that Caldera's internal Data sService does (by writing it to a file on shutdown), and can be cleared in much the same way if necessary (by using the `--fresh` argument on the server).

The following methods are present and supported in the Knowledge Service:
```python
...
|`KnowledgeService.get_fact_origin(fact)`| Identifies the location/source of a provided fact. The `fact` argument can be either a trait to search for or a full blown Fact object. The return is a tuple of the ID corresponding to the origin object for this fact, and the type of origin object.|
```python 
